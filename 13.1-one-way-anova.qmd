...
If the one-way ANOVA is statistically significant, we usually need follow-up analyses, such as planned contrasts or post hoc tests, to determine which groups differ. These follow-up analyses are covered in [Section 13.2: Finding Group Differences](13.2-finding-group-differences.qmd).
...
|------------------------------------|------------------------------------|
| Normality is met and homogeneity of variance is met | One-way ANOVA |
| Normality is met but homogeneity of variance is not met | Welch’s ANOVA |
| Normality is not met | Kruskal-Wallis test |
...
Directional predictions can be tested with planned contrasts or interpreted through follow-up comparisons, which are covered in [Section 13.2: Finding Group Differences](13.2-finding-group-differences.qmd).
...
6.  Ignore the **Model**, **Contrasts**, and **Post Hoc Tests** menus for now. Follow-up analyses are covered in [Section 13.2: Finding Group Differences](13.2-finding-group-differences.qmd).
...
To conduct the Kruskal-Wallis test in jamovi, go to **ANOVA** and choose **One-Way ANOVA, Kruskal-Wallis**. Move `mood.gain` to the **Dependent Variables** box and `drug` to the **Grouping Variable** box. Select `Effect size`. If follow-up comparisons are needed, use `DSCF pairwise comparisons`, which are discussed in [Section 13.2: Finding Group Differences](13.2-finding-group-differences.qmd).
