...
Regression is also connected to many of the statistical tests we have already learned. A regression with one dichotomous predictor produces results that are closely related to an independent *t*-test. A regression with one categorical predictor with three or more levels is closely related to ANOVA. A regression with one continuous predictor is closely related to correlation. We will return to this larger connection in @sec-general-linear-model.
...
As reviewed in @sec-apa-style, an APA-style results section should describe the research question, report the model fit, report the relevant coefficients, and interpret the result.
