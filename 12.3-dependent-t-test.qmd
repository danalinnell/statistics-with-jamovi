...
5.  Under **Additional Statistics**, select `Mean difference`, `Effect size`, and `Descriptives`. You may also select `Descriptives plots`, although Chapter 6 provides better options for creating graphs.
...
A line plot can show change from the first measurement to the second measurement when the data are in a format that supports that graph. A plot of the difference scores can also be useful, especially when checking the normality assumption. A table of the two means and standard deviations is often enough when the focus is the hypothesis test rather than the individual pattern of change.

For this example, a simple graph should communicate that students tended to score higher on the second test than on the first test. Avoid using a graph that makes the two measurements look like independent groups, because the scores are paired within the same students.
